...
class MachineDataManager {
    ...
    static func setMachine() -> Machine {
        ...
        if machineData != nil {
            ...
            let someStrawBerryMilk = BeverageFactory.makeBeverageWithType(brand: "남양유업", price: .low, name: "딸기에몽", packageTime: .fresh, beverageType: StrawBerryMilk.self)
            let someBananaMilk = BeverageFactory.makeBeverageWithType(brand: "빙그레", price: .low, name: "바나나맛우유", packageTime: .fresh, beverageType: BananaMilk.self)
            ...
            let someAmericano = BeverageFactory.makeBeverageWithType(brand: "스타벅스", price: .low, name: "아메리카노-톨", packageTime: .fresh, beverageType: Americano.self)
